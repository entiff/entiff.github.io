...
우선 jupyter notebook config 파일이 있는지 확인합니다.

```
ls ~/.jupyter/jupyter_notebook_config.py
```

없으면 `jupyter notebook --generate-config` 를 통해 생성합니다.

이제 이 config 파일을 수정하겠습니다. `vi` 명령어로 해당 파일을 편집합니다.

```
vi ~/.jupyter/jupyter_notebook_config.py
```

그러면 다음과 같은 화면이 뜹니다. gcp13
...
이제 vm instance에서 pip install ~ 또는 conda install ~을 통해 원하는 패키지를 설치하고 사용하시면 됩니다.
