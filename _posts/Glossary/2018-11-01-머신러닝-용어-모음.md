...
$$ y\quad =\quad \beta x\quad +\quad \varepsilon $$

### 1-2. Multi Linear Regression(중다 회귀, 다중 회귀)

$$ y\quad =\quad { \beta  }_{ 1 }{ x }_{ 1i }\quad +\quad ...\quad +\quad { \beta  }_{ i }{ x }_{ ip }\quad +\quad \varepsilon \quad =\quad { X }_{ i }^{ T }\beta \quad +\quad { \varepsilon  }_{ i } $$
...
선형 모형을 y\quad =\quad ax\quad +\quad b, 연결함수를 g()라 가정할 때,
g(y)\quad =\quad ax\quad +\quad b로 바꾸어 g(y)와 x의 관계를 선형적으로 해석합니다.
y와 x의 관계를 선형적이지 않고 Link Function인 g()에 의해 바뀌기 때문에 주의해야 합니다.
...
(로지스틱 함수)[https://upload.wikimedia.org/wikipedia/commons/thumb/8/88/Logistic-curve.svg/480px-Logistic-curve.svg.png]
